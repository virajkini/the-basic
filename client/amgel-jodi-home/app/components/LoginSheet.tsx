...
export default function LoginSheet({ isOpen, onClose }: LoginSheetProps) {
  ...
  const { signIn: handleGoogleLogin } = useGoogleSignIn({
                ...
                {!!process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID && (
                  <div className="mt-4">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <span className="text-xs text-myColor-400 font-medium">or</span>
                      <div className="flex-1 h-px bg-myColor-100" />
                    </div>
                    <button
                      type="button"
                      ...
                      disabled={loading}
                      className="w-full py-3.5 bg-white border-2 border-myColor-100 rounded-xl font-semibold text-myColor-800 hover:bg-myColor-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-3 shadow-sm"
                    >
                      <GoogleIcon />
